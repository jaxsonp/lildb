...
impl Database {
	pub fn new(name: &str) -> Result<Database, Error> {
		...
		Ok(Self {
			...
			disk_mgr: DiskManager::new(name, Path::new("/var/lib/lildb/"))?,
		})
	}
	...
	fn validate_db_name(name: &str) -> Result<(), Error> {
		...
		{
			Err(Error::new(ValidationError, format!("Invalid database name '{name}': Name must only contain letters, numbers, dashes, and underscores")))
